...
if index_name not in [index.name for index in pc.list_indexes()]:
    pc.create_index(
        name=index_name,
        dimension=384,   # assuming 384 dimensions for the HuggingFace model
...
def create_vector_store(text_chunks):
    ...
    # Connect to Pinecone index with the new syntax
    vector_store = Pinecone.from_texts(
        texts=text_chunks,
        embedding_model=embeddings,
        index_name=index_name,
        pinecone_instance=pc  # Pass the instance here
    )
    return vector_store
